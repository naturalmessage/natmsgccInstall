...
#
# ON FREEBSD, IF PYTHON WAS NOT INSTALLED FROM SOURCE, THE FIRST MESSAGE
...
def nm_popen(cmd_list, wrk_dir, env_dict=None):
    ...
    env_dict is a Python dictionary object that defines environment
    variables, such as:
        ...
        if ssl.OPENSSL_VERSION_INFO[1] == 0:
            if ssl.OPENSSL_VERSION_INFO[2] == 1:
                if ssl.OPENSSL_VERSION_INFO[3] < 10:
                    print('OpenSSL might be out of date: '
                        + ssl.OPENSSL_VERSION)
...
def install_targz_py(wrk_dir, targz_url, proj_name, 
    ...
    """Install Python tar.gz files from github using setup.py.
    ...
    extension.  The target directory for the tar extraction
    is determined by reading the tar file, so I do no
    need any separate indicator of the version of package.
    ...
    except that the Python command is whatever version of python is
    running now (as reported by sys.executable), and if
...
def download_tar_bz2(wrk_dir, tarbz_url ):
  ...
  This assumes that the tar file has a particular
    ...
    if os.path.isfile(tarfile_path):
        ...
        if flen > 0:
            print('It looks like the file has already '
                + 'been downloaded: ' + tarfile_path)
...
def get_dist_name():
    ...
    if platform.system().lower() == 'windows':
        ...
        print('unexpected system type (in brackets): <' 
            + platform.system() + '>.')
        return((None, None))
...
def nm_install_package(package_name, os_name=None, 
    ...
    operating system using the default package manager
    (such as apt-get, yum, pkg, zypper...).  This will use the
    Python 'platform' module to identify the name of the
    operating system, and from that I will guess the name of
    ...
    for the Natural Message client, and that includes a
    translation table so that a request to install 'python3'
    can be converted to the exact package name on each system.
    That lookup comes from a hard-coded list of installable packages.
    ...
    if dist_name is None:
        print('Error. Unexpected system type.  This will probably work '
            + 'only if the system name or Linux distribution name has '
            + 'been coded into this program, but you could try to set '
            + 'the dist_name to something similar to what it is (e.g., '
            + 'if your system is a derivative of debian, enter "debian"'
            + 'without the quotes).')
        dist_name = input('Enter Ctl-c to quit or enter a distribution name '
            + 'if you want to attempt to continue (examples: debian, '
            + 'fedora, opensuse, freebsd, centos linux, openbsd): ').lower()
    ...
    if dist_name in ['openbsd']:
        if os.getenv('PKG_PATH') is None:
            ...
            print('then you can ignore this warning, otherwise, the ')
            print('missing path might cause the pkg command to fail.')
            print('You could try setting the variable and then restarting')
            print('this program:')
            print('   export  PKG_PATH=ftp://ftp.openbsd.org/pub/'
                + 'OpenBSD/5.6/packages/`machine -a`/')
    ...
    if verbosity > 3:
        # change to debug_msg()
        print('Initial pacmgr name based on distribution name: ' \
            + str(pacmgr_name))
    #    
    ...
    while not pacmgr_good:
        ...
        if pacmgr_name is not None:
            ...
            for pp in my_paths:
                if os.path.isdir(pp):
                    if verbosity > 4:
                        print('Scanning for package manager program in '
                            + 'custom paths ' + pp)
...
def get_paths():
    ...
    if os.getlogin() == 'root':
        ...
        while not good:
            ...
            home_dir = input('Enter the full path to the user\'s home '
                + 'directory where you want to install the Natural '
                + 'Message mail directory (must be in the home directory): ')
        ...
        home_dir = os.path.expanduser(os.path.join(
            '~',
            'natmsg',
            'natmsginst'))

    wrk_dir = os.path.expanduser(os.path.join(
        home_dir,
        'natmsg',
        'natmsginst'))

    return((home_dir, wrk_dir))
...
def install_dependencies(url_windows_pycrypto, pycrypto_version):
    ...
    if platform.system().lower() == 'windows':
        ...
        if need_download:
            ...
            rc = xid.wait()
            if rc != 0:
                print('Error. The installer for pycrypto did not run.  '
                    + 'Try running it yourself:')
                print(run_fname)
                print('Try looking for a different version of pycrypto or '
                    + 'install a different version of Python.')
                print('pycrypto binaries for Windows are here:')
                print('  http://www.voidspace.org.uk/python/modules.shtml'
                    + '#pycrypto')
                print(' Compile on your own if you have a C compiler: '
                    + 'https://flintux.wordpress.com/2014/04/30/'
                    + 'pycrypto-for-python-3-4-on-windows-7-64bit/')
                print('Download binaries: ' \
                    + 'https://www.dropbox.com/s/n6rckn0k6u4nqke/'
        ...
        if dist_name not in ['openbsd', 'freebsd']:
            ...
            # of python.
            #
            # The exact pkg name is translated by the func.
            nm_install_package('python3')
        ...
        if need_download:
            if dist_name in ['centos linux']:
                ...
                print('Error. You do not have setuptools, and there is no ')
                print('RPM for Python 3 setuptools in Cent OS 7. You should ')
                print('have already run the NatMsgccSetupCentOS7.sh script ')
                print('in:')
                print('https://github.com/naturalmessage/natmsgccInstall/'
                    + 'archive/master.zip')
                print('That script installs Python 3, dependencies, and ')
                print('setuptools from source.')
                ...
                print('A test of Mageia in early 2015 showed that it had an ')
                print('old version of openssl that is incompatible with ')
                print('this version of the setup routine.')
                ##wget https://bootstrap.pypa.io/ez_setup.py
                dat = https_download(
                    the_url='https://bootstrap.pypa.io/ez_setup.py')
                if dat is None:
                    print('failed to install setuptools.  '
                        + 'The other installs will')
                    print('probably fail.')
                    return(234234)
                ...
                try:
                    ...
                    print('failed to run setuptools installer.  '
                        + 'The other installs will')
                    print('probably fail.')
                    return(87878787)
                else:
                ...
                if rc != 0:
                    input('The installation of python setuptools failed.  '
                        + 'This will adversely affect installation of the '
                        + 'Natural Message command line client.  '
        ...
        and dist_name == 'freebsd':
            ...
            print('WARNING: I did not find the gcc program, but I will ')
            print('install gcc48 if need be.  The FreeBSD install will ')
            print('not create a file called gcc, so you might want to '
                + 'create a soft link to the correct version of gcc:')
        ...
        and dist_name != 'freebsd':
            # Removes rtf code for command line viewing
            nm_install_package('unrtf')

    return(0)

###############################################################################
def  install_important_stuff(
        wrk_dir,
        url_pycrypto,
        url_requests,
        url_natmsgcc,
    ...
    # Get the fresh installer to facilitate running the latest install:
    steps.append([
        os.curdir,
        url_natmsgccInstall,
        'natmsgccInstall',
    ...
    if dist_name == 'freebsd':
        ...
        if not os.path.isfile('/usr/bin/unrtf') \
                and not os.path.isfile('/usr/bin/unrtf'): 
        ...
        if root_owner_found:
            if os.getlogin() == 'root':
                print('WARNING, your mail directory contains files that are '
                    ...
                    + 'accessible by your regular user ID.  Your login ID '
                    + 'is also set to root. ')
                owner_id_alpha = input('Enter the user ID that you want ' \
                    + 'to be the owner of your mail directory: ')
...
def install_libgcrypt(
    ...
    #
    # needs: wrk_dir, url_libgpg_error, url_libgcrypt, proj_dir
    if platform.system().lower() != 'windows' and dist_name !='freebsd':
        ...
        if os.path.isfile('/usr/local/lib/libgpg-error.a'):
            ...
            if rc == 0:
                ...
                if rc != 0:
                    input('Error.  There was an error while '
                        + 'configuring libgpg_error. '
                        + 'You might need to install a dependency.')
                ...
                if rc != 0:
                    input('Error.  There was an error while configuring '
                        + 'libgpg_error. You might need to install a '
                        + 'dependency.')
            
                ## # make install
                print('The libgpg-error modules are needed by the libgcrypt'
                    + 'module.')
                yn = input('Do you want to install the libgpg-error '
                    + 'programs now? (y/n): ')
                if yn.lower() in ['y', 'yes']:
                    pid = None
        ...
        if os.path.isfile('/usr/local/lib/libgcrypt.a'):
            ...
            if rc == 0:
                ...
                if rc != 0:
                    input('Error.  There was an error while configuring ' \
                        + 'libgcrypt. You might need to install a dependency.')
                ...
                # make install
                yn = input('Do you want to install the libgcrypt '
                    + 'programs now? (y/n): ')
                if yn.lower() in ['y', 'yes']:
                    print('Installing libgpg_error...')
                    rc = nm_popen(['make', 'install'], proj_dir)
                    ...
                    if rc !=0:
                        print('Error.  There was an error while making '
                            + 'libgcrypt. You might ')
...
def install_natmsgv(wrk_dir, url_natmsgv_gz):
    ...
    if platform.system().lower() != 'windows':
        ...
        err_nbr, proj_subdir = install_targz_py(
            wrk_dir,
            url_natmsgv_gz,
            'natmsgv-master',
            False, False)
        if err_nbr != 0:
            # Error/warning
            print('Error.  There was an error intalling the '
                + 'server verification tar.gz file: ' + str(err_nbr))
        else:
            print('The natmsgv server verification program source '
                + 'code was downloaded OK.')

            if dist_name == 'freebsd':
                # rename the Makefile
                mfile = os.path.join(wrk_dir, 'natmsgv-master', 'Makefile')
                fbsd_file= os.path.join(
                    wrk_dir,
                    'natmsgv-master',
                    'MakefileFreeBSD')
            ...
            rc = nm_popen(
                ['make', 'nm_verify'],
                os.path.join(wrk_dir, 'natmsgv-master'))

            if rc is None:
                input('Error.  There was an error while making natmsgv. '
                    + 'You might need to install a dependency.')
            elif rc != 0:
                input('Error.  There was an error while making natmsgv. '
                    + 'You might need to install a dependency.')
                ...
                if rc != 0:
                    input('Error.  There was an error while installing '
                        + 'nm_verify. You might ' \
...
# run it
def main():
    global URL_WINDOWS_PYCRYPTO
    global URL_REQUESTS
    global URL_NATMSGCC 
    global URL_RNCRYPTOR
    ...
    #### A user guide:
    print('The current way to run this program is to run the launcher '
